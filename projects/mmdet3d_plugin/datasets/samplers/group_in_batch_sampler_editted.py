...
class GroupInBatchSampler(Sampler):
    ...
    def __init__(
        ...
        if hasattr(self.dataset, 'datasets'):  # If ConcatDataset
            # Concatenate flags from all sub-datasets
            self.flag = np.concatenate([d.flag for d in self.dataset.datasets])
        else:
            self.flag = self.dataset.flag
        self.group_sizes = np.bincount(self.flag)
        self.groups_num = len(self.group_sizes)
        self.global_batch_size = batch_size * world_size
        print(f"[DEBUG] Dataset type inside sampler: {type(self.dataset)}")
        if hasattr(self.dataset, 'datasets'):
            print(f"[DEBUG] ConcatDataset with {len(self.dataset.datasets)} subdatasets")
        print(
        f"[DEBUG] GroupInBatchSampler assertion check:\n"
        f"  groups_num = {self.groups_num}\n"
        f"  global_batch_size = {self.global_batch_size}\n"
        f"  batch_size = {self.batch_size}\n"
        f"  world_size = {self.world_size}\n"
        f"  dataset length = {len(self.dataset)}\n" 
        )
        print("[DEBUG] Dataset roots and ann_files:")
        for i, d in enumerate(self.dataset.datasets):
            print(f"  {i}: root={getattr(d, 'data_root', None)}, ann={getattr(d, 'ann_file', None)}")
        ...
        while True:
            ...
            for local_sample_idx in range(self.batch_size):
                ...
                if len(self.buffer_per_local_sample[local_sample_idx]) == 0:
                    ...
                    base_ds = getattr(self.dataset, 'datasets', [self.dataset])[0]
                    keep_seq_aug = getattr(self.dataset, 'keep_consistent_seq_aug',
                                        getattr(base_ds, 'keep_consistent_seq_aug', False))

                    # Choose the function that returns augmentation state
                    get_aug = getattr(self.dataset, 'get_augmentation',
                                    getattr(base_ds, 'get_augmentation', None))

                    # Case 1: if keep_consistent_seq_aug is True → reuse per-sample augmentation
                    if keep_seq_aug:
                        if get_aug is not None:
                            aug_state = get_aug()
                        else:
                            aug_state = None
                        self.aug_per_local_sample[local_sample_idx] = aug_state

                    # Case 2: otherwise generate new augmentation state each sample
                    else:
                        if get_aug is not None:
                            aug_state = get_aug()
                        else:
