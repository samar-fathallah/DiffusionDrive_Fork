...
class NuScenesSparse4DAdaptor(object):
    ...
    def __call__(self, input_dict):
        print("=== DEBUG NuScenesSparse4DAdaptor ===")
        print("  type(img_shape):", type(input_dict.get("img_shape")))
        print("  img_shape value:", input_dict.get("img_shape"))
        input_dict["projection_mat"] = np.float32(
            np.stack(input_dict["lidar2img"])
        )
        arr = np.array(input_dict["img_shape"], dtype=np.float32)
