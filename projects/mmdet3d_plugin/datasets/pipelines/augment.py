...
class ResizeCropFlipImage(object):
    def __call__(self, results):
        # ---- ENTER: show what came from the loader ----
        print("=== DEBUG ResizeCropFlipImage (enter) ===")
        print("  keys:", list(results.keys()))
        print("  type(results['img']):", type(results.get("img")))
        if isinstance(results.get("img"), list) and len(results["img"]) > 0:
            print("  first img pre shape:", results["img"][0].shape)   # (H,W,3)
        print("  results['img_shape'] (pre):", results.get("img_shape"))  # expected (H,W,C,6)
        print("  results['lidar2img'] len:", len(results.get("lidar2img", [])))
        print("  results['cam_intrinsic'] present:", "cam_intrinsic" in results)

        aug_config = results.get("aug_config")
        print("  aug_config:", aug_config)

        # If there's no aug_config (e.g., some eval stages), bail out early.
        if aug_config is None:
            print("=== DEBUG ResizeCropFlipImage (skip: no aug_config) ===")
            return results
        ...
        for i in range(N):
            # Each img is HWC (BGR) here
            img_i = np.uint8(imgs[i])

            # Optionally log per-view input sizes
            print(f"    [view {i}] pre shape: {img_i.shape}")

            # Apply your transform (resize/crop/flip/rotate), returning the 4x4 warp mat
            img_out, mat = self._img_transform(img_i, aug_config)
            img_out = np.asarray(img_out, dtype=np.float32)
            new_imgs.append(img_out)

            # Update extrinsics/projection
            results["lidar2img"][i] = mat @ results["lidar2img"][i]

            # Update intrinsics for pure resize (your code matches DiffusionDrive’s test-time path)
            if "cam_intrinsic" in results:
                results["cam_intrinsic"][i][:3, :3] *= aug_config["resize"]

            # Log post-view stats
            print(f"    [view {i}] post shape: {img_out.shape}")
            print(f"    [view {i}] mat (top-left 2x3):\n{mat[:2,:3]}")

        # Store transformed images + per-view shapes
        results["img"] = new_imgs
        results["img_shape"] = [x.shape[:2] for x in new_imgs]  # [(H,W), ...]
        print("  results['img_shape'] (post):", results["img_shape"])

        print("=== DEBUG ResizeCropFlipImage (exit) ===")
        return results
