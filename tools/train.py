...
def main():
    ...
    #     if "motion_plan_head" in name:
    #         print(name, param.requires_grad)
    #     param.requires_grad = False

    # for name, param in model.head.motion_plan_head.named_parameters():
    #     if 'diff_refine' in name or 'planning_decoder' in name or 'refine' in name:
    ...
    train_ds = datasets[0]
    print("\n[DEBUG] Training dataset type:", type(train_ds))
    try:
        # If it's a ConcatDataset
        print("[DEBUG] Total length:", len(train_ds))
        if hasattr(train_ds, "datasets"):
            for i, d in enumerate(train_ds.datasets):
                print(f"[DEBUG]  Subdataset {i}: len={len(d)}, root={getattr(d, 'data_root', 'N/A')}")
    except Exception as e:
        print("[DEBUG] Could not print dataset info:", e)
    print("=====================================\n")
    # ===================================
